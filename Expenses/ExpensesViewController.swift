...
class ExpensesViewController: UIViewController {
   ...
   let fetchRequest: NSFetchRequest<Expense> = Expense.fetchRequest()
        //fetch is a generic can behave with any type as long they meet certly requieres
        do {
        expenses = try managedContext.fetch(fetchRequest)
            //has to be wrapp in a do "lets cacth the exeption"
            expensesTableView.reloadData()
            //if the fectch is catch because is not new data so id going to print
        } catch {
...
extension ExpensesViewController: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        //accesing the epense array wwe should have a singe row for expense
    }
